...
The model produces an AUC of .722, with a sensitivity of .66 and a specificity of .65.  This model is ok, and better than a baseline prediction of .5 - but is not neccisarily a model you would want to make investment decisions on. 
...
The glmnet model is comparabe to the standard glm model with an AUC of .722. 
...
The boosted logistic regression is also incrementally better than the previous two models.  The AUC is .712. While this is better, its not spectacular for investment grade decision making. 
...
The ctree model returned an AUC of .74 with a sensitivity of .67 and specificity of .69. This model has the best quality thus far.  
...
The CART model returned an AUC of .772 with a sensitivity of .67 and specificity of .72. This model is of reasonably higher quality than the previous regression based models.   
...
The random forest model produces a very highly quality prediction.  It is computationally expensive and takes a long time to process, however - this is clearly a reasonably high quality prediction.  The AUC comes back as .851 with a sensitivity of .74 and specificity of .78.  This score should be usable for our loan predictions. 
...
The GBM model appears highly performance with a AUC of just under .82!. This is much better than the regression based models. 
...
The GBM model performed well, with an accuracy of 74%.  The sensivity was .70 and the specificity was .77.  THe AUC for predicted values was .7281.  What is troubling is that the model still predcited that 1139 loans would be repaid, but in fact defaulted.  This is a very high false negative rate of 14.7% which would result in a fairly substantial loss of principal.  By adjusting the cutoff value, it would be likely that we could imporive this false negative rate, as false positives would inclur lower losses for the investor.  
