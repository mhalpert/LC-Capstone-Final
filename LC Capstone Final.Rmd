...
RawLoanData <- SMOTE(loan_status ~ ., RawLoanData, k = 5, perc.over = 100, perc.under=200)
...
The model produces an AUC of .714, with a sensitivity of .66 and a specificity of .65.  This model is ok, and better than a baseline prediction of .5 - but is not neccisarily a model you would want to make investment decisions on. 
...
The glmnet model is incrementally better than the standard glm model with an AUC of .714. 
...
The boosted logistic regression is also incrementally better than the previous two models.  The AUC is .725. While this is better, its not spectacular for investment grade decision making. 
...
The ctree model returned an AUC of .73 with a sensitivity of .64 and specificity of .69. This model has the best quality thus far.  
...
The CART model returned an AUC of .767 with a sensitivity of .67 and specificity of .72. This model is of reasonably higher quality than the previous regression based models.   
...
The random forest model produces a very highly quality prediction.  It is computationally expensive and takes a long time to process, however - this is clearly a reasonably high quality prediction.  The AUC comes back as .848 with a sensitivity of .73 and specificity of .78.  This score should be usable for our loan predictions. 
...
The GBM model appears highly performance with a AUC of just under .820. This is much better than the regression based models. 
...
To complete our analysis we are going to test the accuracy of the models against our 30% out of sample data set.
...
The GBM model performed well, with an accuracy os 73%.  The sensivity was .67 and the specificity was . 79.  THe AUC for predicted values was .7281.  What is troubling is that the model still predcited that 1286 loans would be repaid, but in fact defaulted.  This is a very high false negative rate of 16.6% which would result in a fairly substantial loss of principal.  By adjusting the cutoff value, it would be likely that we could imporive this false negative rate, as false positives would inclur lower losses for the investor.  
...
The Random Forest model performed well, with an accuracy os 75%.  The sensivity was .72 and the specificity was .79.  The AUC was .7557.  While the accuracy was only slightly higher than the GBM model predictions, the false negative rate was much lower with 14.14% with 1090 predicted repaid loans defaulting.  While better than GBM, this is also still a very high false negative rate which would result in a fairly substantial loss of principal.  By adjusting the cutoff value, it would be likely that we could imporive this false negative rate, as false positives would inclur lower losses for the investor.  
